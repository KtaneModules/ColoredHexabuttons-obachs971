...
public class greenHexabuttons : MonoBehaviour
{
	...
	void Awake()
	{
		...
		Debug.LogFormat("[Green Hexabuttons #{0}] Generated Measure: {1}", moduleId, (measure + 1));
		centerText.text = (measure + 1) + "";
		solution = sheetMusic[measure].ToUpperInvariant();
		Debug.LogFormat("[Green Hexabuttons #{0}] Notes in measure: {1} {2} {3} {4} {5} {6}", moduleId, "-0123456789A".IndexOf(solution[0]), "-0123456789A".IndexOf(solution[1]), "-0123456789A".IndexOf(solution[2]), "-0123456789A".IndexOf(solution[3]), "-0123456789A".IndexOf(solution[4]), "-0123456789A".IndexOf(solution[5]));
		greenNotes = new string(solution.ToCharArray().Shuffle());
		foreach (int i in buttonIndex)
		{
			greenNotes = greenNotes + "" + greenNotes[i];
			hexButtons[i].OnInteract = delegate { pressedButton(i); return false; };
			hexButtons[i].OnInteractEnded = delegate { releasedButton(i); };
		}
		Debug.LogFormat("[Green Hexabuttons #{0}] Notes on buttons in reading order: {1} {2} {3} {4} {5} {6}", moduleId, "-0123456789A".IndexOf(greenNotes[0]), "-0123456789A".IndexOf(greenNotes[1]), "-0123456789A".IndexOf(greenNotes[2]), "-0123456789A".IndexOf(greenNotes[3]), "-0123456789A".IndexOf(greenNotes[4]), "-0123456789A".IndexOf(greenNotes[5]));
	...
	{
		Debug.LogFormat("[Green Hexabuttons #{0}] User submitted {1}, which plays the note {2}.", moduleId, positions[n], "-0123456789A".IndexOf(greenNotes[n]));
		Audio.PlaySoundAtTransform(notes["0123456789A".IndexOf(greenNotes[n])].name, transform);
...
#pragma warning restore 414
	...
	IEnumerator TwitchHandleForcedSolve()
    {
		...
		while (flag)
        {
			hexButtons[Array.IndexOf(greenNotes.ToCharArray(), solution[numButtonPresses])].OnInteract();
			yield return new WaitForSeconds(0.3f);
		}
    }
}
